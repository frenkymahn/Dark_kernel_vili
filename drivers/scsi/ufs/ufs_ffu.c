...
#include "wdc_64GB_128GB_2008.h"
#include "YMTC_UFS22_3_6.h"
...
#define PART_NAME 						"sdc"
#define PART_NUMBER 					(62)
...
extern int ffu_set_flag(int flag) __attribute__((weak));
extern int ffu_get_flag(void) __attribute__((weak));
static partinfo part_info = { 0 };
static bool one_time_entry = false;
static bool off_charge_flag = false;
static struct fw_update_checklist need_update_k[] = {
	...
	{"WDC", "SDINDDK4-64", "2000","2008",
	(uint64_t)wdc_64GB_128GB_2008, sizeof(wdc_64GB_128GB_2008)},
	{"WDC", "SDINDDK4-128", "2000","2008",
	(uint64_t)wdc_64GB_128GB_2008, sizeof(wdc_64GB_128GB_2008)},
    {"YMTC", "YMUS8B2TE2A2C1", "3.2","3.6",
	(uint64_t)YMTC_UFS22_3_6, sizeof(YMTC_UFS22_3_6)},
    {"YMTC", "YMUS8B2TE2A2C1", "3.4","3.6",
	(uint64_t)YMTC_UFS22_3_6, sizeof(YMTC_UFS22_3_6)},
};
...
int ufs_ffu(struct scsi_device *sdev)
{
	...
	pr_info("Enter ufs ffu function, pmic flag:0x%x\n", ffu_get_flag());
	if (one_time_entry) {
	...
	if (ffu_get_flag) {
		if (ffu_get_flag() == 1) {
			pr_info("[ufs_ffu] fail flag was set in last boot. skip FFU.\n");
			ret = ffu_set_flag(0);
			ret = -1;
			return ret;
		}
	} else {
		pr_info("[ufs_ffu] fail flag is not supported\n");
	}
		...
		if (strncmp((char *)stdinq.vendor_id, (char *)need_update[i].vendor_id, strlen(need_update[i].vendor_id)) == 0
			...
			if (strncmp((char *)stdinq.product_rev, (char *)need_update[i].product_rev_from, strlen(need_update[i].product_rev_from)) == 0) {
				...
				if (ret == 0) {
					...
					msleep(10);
					machine_restart("ffu reboot");
					pr_info("should not get here.\n");
				} else {
					/*set a flag at here and do not enter ffu next time.*/
					if (ffu_set_flag) {
						pr_info("ffu failed, don't clear fail flag, reboot directly\n");
						msleep(10);
						machine_restart("ffu reboot");
					}
					pr_info("do not reboot to avoid endless reboot\n");
				}
				break;
