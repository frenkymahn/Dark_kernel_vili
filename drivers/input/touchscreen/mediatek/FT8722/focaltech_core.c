...
int fts_reset_proc(int hdelayms)
{
		...
		FTS_INFO("[ESD]atomic_set(&is_lcm_inited_esd,0)\n");
	#endif
	...
	if (hdelayms) {
		...
		FTS_INFO("[ESD]atomic_set(&is_lcm_inited_esd,1)\n");
	#endif
...
static int fts_ts_suspend(struct device *dev)
{
	...
	//if (fts_data->palm_sensor_switch) {
		FTS_INFO("%s: palm sensor off status, switch to off\n", __func__);
		update_palm_sensor_value(0);
