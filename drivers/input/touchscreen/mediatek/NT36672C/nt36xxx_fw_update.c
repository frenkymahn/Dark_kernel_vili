...
static struct nvt_ts_bin_map *bin_map_mp;

static int32_t nvt_update_firmware_mp(char *firmware_name);
...
static int32_t nvt_bin_header_parser_mp(const u8 *fwdata, size_t fwsize)
{
	uint32_t list = 0;
	uint32_t pos = 0x00;
	uint32_t end = 0x00;
	...
	NVT_LOG("ovly_info = %d, ilm_dlm_num = %d, ovly_sec_num = %d, info_sec_num = %d, partition = %d\n",
			ovly_info, ilm_dlm_num, ovly_sec_num, info_sec_num, partition);
	/* allocated memory for header info */
	bin_map_mp = (struct nvt_ts_bin_map *)kzalloc((partition+1) * sizeof(struct nvt_ts_bin_map), GFP_KERNEL);
	if (bin_map_mp == NULL) {
		NVT_ERR("kzalloc for bin_map_mp failed!\n");
		return -ENOMEM;
	}
	for (list = 0; list < partition; list++) {
		/*
		 ...
		 */
		if (list < ilm_dlm_num) {
			bin_map_mp[list].BIN_addr = byte_to_word(&fwdata[0 + list*12]);
			bin_map_mp[list].SRAM_addr = byte_to_word(&fwdata[4 + list*12]);
			bin_map_mp[list].size = byte_to_word(&fwdata[8 + list*12]);
			if (ts->hw_crc)
				bin_map_mp[list].crc = byte_to_word(&fwdata[0x18 + list*4]);
			else { //ts->hw_crc
				if ((bin_map_mp[list].BIN_addr + bin_map_mp[list].size) < fwsize)
					bin_map_mp[list].crc = CheckSum(&fwdata[bin_map_mp[list].BIN_addr], bin_map_mp[list].size);
				else {
					NVT_ERR("access range (0x%08X to 0x%08X) is larger than bin size!\n",
							bin_map_mp[list].BIN_addr, bin_map_mp[list].BIN_addr + bin_map_mp[list].size);
					return -EINVAL;
				}
			} //ts->hw_crc
			if (list == 0)
				sprintf(bin_map_mp[list].name, "ILM");
			else if (list == 1)
				sprintf(bin_map_mp[list].name, "DLM");
		}
		/*
		 * [2] parsing others header info
		 * SRAM_addr : size : BIN_addr : crc (16-bytes)
		...
		if ((list >= ilm_dlm_num) && (list < (ilm_dlm_num + info_sec_num))) {
			/* others partition located at 0x30 offset */
			pos = 0x30 + (0x10 * (list - ilm_dlm_num));
			bin_map_mp[list].SRAM_addr = byte_to_word(&fwdata[pos]);
			bin_map_mp[list].size = byte_to_word(&fwdata[pos+4]);
			bin_map_mp[list].BIN_addr = byte_to_word(&fwdata[pos+8]);
			if (ts->hw_crc)
				bin_map_mp[list].crc = byte_to_word(&fwdata[pos+12]);
			else { //ts->hw_crc
				if ((bin_map_mp[list].BIN_addr + bin_map_mp[list].size) < fwsize)
					bin_map_mp[list].crc = CheckSum(&fwdata[bin_map_mp[list].BIN_addr], bin_map_mp[list].size);
				else {
			...
			/* detect header end to protect parser function */
			if ((bin_map_mp[list].BIN_addr == 0) && (bin_map_mp[list].size != 0)) {
				sprintf(bin_map_mp[list].name, "Header");
			} else {
				sprintf(bin_map_mp[list].name, "Info-%d", (list - ilm_dlm_num));
			}
		}
		/*
		 * [3] parsing overlay section header info
		...
		if (list >= (ilm_dlm_num + info_sec_num)) {
			/* overlay info located at DLM (list = 1) start addr */
			pos = bin_map_mp[1].BIN_addr + (0x10 * (list - ilm_dlm_num - info_sec_num));
			bin_map_mp[list].SRAM_addr = byte_to_word(&fwdata[pos]);
			bin_map_mp[list].size = byte_to_word(&fwdata[pos+4]);
			bin_map_mp[list].BIN_addr = byte_to_word(&fwdata[pos+8]);
			if (ts->hw_crc)
			...
			sprintf(bin_map_mp[list].name, "Overlay-%d", (list - ilm_dlm_num - info_sec_num));
		}
		/* BIN size error detect */
		if ((bin_map_mp[list].BIN_addr + bin_map_mp[list].size) > fwsize) {
			NVT_ERR("access range (0x%08X to 0x%08X) is larger than bin size!\n",
...
//		NVT_LOG("[%d][%s] SRAM (0x%08X), SIZE (0x%08X), BIN (0x%08X), CRC (0x%08X)\n",
//				list, bin_map_mp[list].name,
//				bin_map_mp[list].SRAM_addr, bin_map_mp[list].size,  bin_map_mp[list].BIN_addr, bin_map_mp[list].crc);
	}
        return 0;
}
/*******************************************************
...
static int32_t update_firmware_request_mp(char *filename)
{
	uint8_t retry = 0;
	int32_t ret = 0;
	if (NULL == filename) {
	...
	while (1) {
		...
		// check FW need to write size
		if (nvt_get_fw_need_write_size(fw_entry_mp)) {
			NVT_ERR("get fw need to write size fail!\n");
			ret = -EINVAL;
			goto invalid;
		}
		// check if FW version add FW version bar equals 0xFF
		if (*(fw_entry_mp->data + FW_BIN_VER_OFFSET) + *(fw_entry_mp->data + FW_BIN_VER_BAR_OFFSET) != 0xFF) {
			NVT_ERR("bin file FW_VER + FW_VER_BAR should be 0xFF!\n");
			NVT_ERR("FW_VER=0x%02X, FW_VER_BAR=0x%02X\n", *(fw_entry_mp->data+FW_BIN_VER_OFFSET), *(fw_entry_mp->data+FW_BIN_VER_BAR_OFFSET));
			ret = -ENOEXEC;
			goto invalid;
		}
		/* BIN Header Parser */
		ret = nvt_bin_header_parser_mp(fw_entry_mp->data, fw_entry_mp->size);
		if (ret) {
			NVT_ERR("bin header parser failed\n");
			goto invalid;
		} else {
...
int32_t nvt_update_firmware(char *firmware_name)
{
	int32_t ret = 0;
        ...
        NVT_ERR("nvt_update_firmware fw_name=%s\n", firmware_name);

	if (!strcmp(firmware_name, "nt36672c_tm_01_ts_mp.bin")){
		ret = nvt_update_firmware_mp("nt36672c_tm_01_ts_mp.bin");
		NVT_ERR("nvt_update_firmware mp (%d)\n", ret);
                return ret;
	}
	...
	if (strcmp(firmware_name, "nt36672c_tm_01_ts_mp.bin") && fw_entry_mp == NULL) {
		ret = update_firmware_request_mp("nt36672c_tm_01_ts_mp.bin");
		NVT_ERR("nvt_update_firmware mp (%d)\n", ret);
	}
download_fail:
	if (!IS_ERR_OR_NULL(bin_map)) {
...
int32_t nvt_update_firmware_mp(char *firmware_name)
{
	int32_t ret = 0;
	/* request bin file in "/etc/firmware"
	ret = update_firmware_request(firmware_name);
	...
	/* initial buffer and variable */
        NVT_ERR("nvt_update_firmware_mp\n");
