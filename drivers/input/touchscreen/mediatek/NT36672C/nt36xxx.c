...
/*L19 code for HQ-159452 by sunfeiting at 20211102 start*/
#include <linux/hqsysfs.h>
/*L19 code for HQ-159452 by sunfeiting at 20211102 end*/
...
void get_tp_info(void)
{
	...
	} else if (strstr(saved_command_line, "dsi_panel_K19_43_02_0b2_vdo")) {
		sprintf(tp_version_info, "[Vendor]TRULY(KN),[TP-IC]:NT36672C,[FW]0x%x,PID=%04X\n", tp_fw_version, ts->nvt_pid);
	}
...
int tp_compare_ic(void)
{
	...
	if (strstr(saved_command_line, "dsi_panel_l19_36_02_0a_dsc_vdo_lcm_drv")) {
		...
		BOOT_UPDATE_FIRMWARE_NAME = "nt36672c_tr_02_kn_ts_fw.bin";
		MP_UPDATE_FIRMWARE_NAME = "nt36672c_tr_02_kn_ts_mp.bin";
		NVT_LOG("match TP_04");
		return 0;
	} else {
...
static int32_t nvt_ts_suspend(struct device *dev)
{
	...
	#ifdef CONFIG_TOUCHSCREEN_XIAOMI_TOUCHFEATURE
	//AM-54712 hide for palm_sensor failure start
	/*if (ts->palm_sensor_switch) {*/
		NVT_LOG("%s: palm sensor off status, switch to off\n", __func__);
		update_palm_sensor_value(0);
		//nvt_palm_sensor_cmd(0);
		ts->palm_sensor_switch = false;
	//	}
	//AM-54712 hide for palm_sensor failure end
	#endif
#if WAKEUP_GESTURE
...
int32_t nvt_ts_tp_suspend(void)
{
	...
	//if (ts->palm_sensor_switch) {
		...
		ts->palm_sensor_switch = false;
	//	}
	//AM-54712 hide for palm_sensor_ failure end
	#endif
#if WAKEUP_GESTURE
	if (nvt_gesture_flag == false)
		nvt_irq_enable(false);
...
static int32_t nvt_ts_resume(struct device *dev)
{
	...
	#ifdef CONFIG_TOUCHSCREEN_XIAOMI_TOUCHFEATURE
	if (ts->palm_sensor_switch) {
		NVT_LOG("%s: palm sensor on status, switch to on\n", __func__);
...
{
	if (!(strcmp(str, "dsi_panel_l19_31_03_0b_dsc_vdo_lcm_drv"))) {
		is_ft_lcm = 1;
