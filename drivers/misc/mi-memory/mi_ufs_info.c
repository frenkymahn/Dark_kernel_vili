...
static int scsi_sdr(struct scsi_device *sdev, char *sdr, int len)
{
	...
	result = __scsi_execute(sdev, cmd, DMA_FROM_DEVICE, sdr,
...
static int scsi_sk_hr(struct scsi_device *sdev, char *buff, int len)
{
	...
	if (!strncmp(product_id, hynix_v7_uMCP_list[0], strlen(hynix_v7_uMCP_list[0])) ||
			!strncmp(product_id, hynix_v7_uMCP_list[1], strlen(hynix_v7_uMCP_list[1])))
			cmd[11] = 0x52; /*for V7 hynix Voyager*/
	else
		cmd[11] = 0x2c; /*for V6 hynix Voyager*/

	pr_err("%s %d cmd[11]:0x%x\n", __func__, __LINE__, cmd[11]);

	result = __scsi_execute(sdev, cmd, DMA_FROM_DEVICE, buff,
                                  len, NULL, NULL, 30 * HZ, 3, 0, RQF_PM, NULL);
...
/* L19 code for HQ-232249 by xukai at 2022/08/17 start */
/**
get ymtc hr
 */
...
/* L19 code for HQ-232249 by xukai at 2022/08/17 end */
...
{
	size = hba->desc_size.health_desc;
...
static int ufs_get_skhynix_hr(struct scsi_device *sdev, u8 *buf, u32 size)
{
	...
	ret = ufs_get_hynix_hr(hba, buf, 0x80);
