...
static struct ddr_data *ddr_data_ptr;
...
{
	if (!ddr_data_ptr) {
		ddr_data_ptr = kzalloc(sizeof(struct ddr_data), GFP_ATOMIC);

		if (!ddr_data_ptr)
			return;

		ddr_data_ptr->dramc_pdev = pdev;
	}
}
...
struct ddr_data *get_ddr_data(void)
{
	return ddr_data_ptr;
}

EXPORT_SYMBOL(get_ddr_data);
...
int ufshcd_read_desc(struct ufs_hba *hba, enum desc_idn desc_id,
...
int ufs_get_string_desc(struct ufs_hba *hba, void* buf, int size, enum device_desc_param pname, bool ascii_std)
{
	...
	ret = ufshcd_read_string_desc(hba, index, &desc_buf,  ascii_std);//kernel升级，函数入参变化

	copy_size = ret > size ? size : ret;

	if (ret < 0)
		goto out;

		memcpy(buf, desc_buf, copy_size);
