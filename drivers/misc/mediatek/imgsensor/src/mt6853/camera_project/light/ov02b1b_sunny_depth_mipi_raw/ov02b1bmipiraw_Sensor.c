...
#define LOG_1 LOG_INF("OV02B1B,MIPI 1LANE\n")
/***************************   Modify end    *******************************/
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = OV02B1B_SUNNY_DEPTH_SENSOR_ID,
...
extern int ov02b1b_otp_read(void);
extern otp_error_code_t OV02B1B_OTP_ERROR_CODE;
 MUINT32  sn_inf_main_ov02b1b[13];  /*0 flag   1-12 data*/
 MUINT32  material_inf_main_ov02b1b[4];
...
static void set_dummy(void)
{
	LOG_INF("dummyline = %d, dummypixels = %d\n",
	    ...
	    write_cmos_sensor(0x14, (imgsensor.frame_length - 0x398) >> 8);
	    write_cmos_sensor(0x15, (imgsensor.frame_length - 0x398) & 0xFF);
...
/*L19 code for HQ-161200 by WangChenXu at 2021/11/17 start*/
...
/*L19 code for HQ-161200 by WangChenXu at 2021/11/17 end*/
...
write_cmos_sensor(0x57,0x0e);
write_cmos_sensor(0x58,0x24);
write_cmos_sensor(0x59,0x20);
write_cmos_sensor(0x5a,0x0e);
...
static void preview_setting(void)
{
	 ...
	 * 100 99 1600 1200
	 */
write_cmos_sensor(0xfd,0x00);  //;crop to 1600x1200
write_cmos_sensor(0x4f,0x06);
write_cmos_sensor(0x50,0x40);  //;mipi hszie low 8bit
write_cmos_sensor(0x51,0x04);
write_cmos_sensor(0x52,0xb0);  //;mipi vsize low 8bit
write_cmos_sensor(0xfd,0x01);
write_cmos_sensor(0x02,0x00);
write_cmos_sensor(0x03,0x70);  //;window hstart low 8bit
write_cmos_sensor(0x04,0x00);
write_cmos_sensor(0x05,0x10);  //;window vstart low 8bit
write_cmos_sensor(0x06,0x03);
write_cmos_sensor(0x07,0x20);  //;window hsize low 8bit
write_cmos_sensor(0x08,0x04);
write_cmos_sensor(0x09,0xb0);  //;window vsize low 8bit
write_cmos_sensor(0x14,0x00);
write_cmos_sensor(0x15,0x24);
write_cmos_sensor(0xfe,0x02);
	mdelay(5);
 ...
 *
 *************************************************************************/
  /*L19 code for HQ-161200 by TianGuchen at 2021/12/29 start*/
  static char otpData[20]={0};
  static kal_int8 ov02b1b_read_general_info(int *addr, kal_uint16 addr_len, kal_uint8 *result)
  {
  	int i = 0;
  ...
  unsigned int ov02b1b_read_otp()
  {
  	int ret = 0;
	kal_uint8 i;
	int layoutcheck_addr[] = {0x12, 0x13,0x14,0x15};
	int fuseid_addr[] = {0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f};
	ret = ov02b1b_read_general_info(layoutcheck_addr, ARRAY_SIZE(layoutcheck_addr), otpData);
	if (ret != 0) {
  		LOG_INF("read layoutcheck failed\n");
  		return -1;
  	}
	ret = ov02b1b_read_general_info(fuseid_addr, ARRAY_SIZE(fuseid_addr), otpData+ARRAY_SIZE(layoutcheck_addr));
  	if (ret != 0) {
  ...
  unsigned int ov02b1b_read_otp_info(struct i2c_client *client,
...
/*L19 code for HQ-161200 by TianGuchen at 2021/12/29 end*/
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	int ret = 0;
	ret  = ov02b1b_read_otp();
	if (ret != 0) {
		pr_err("Read ov02b1b sunny otp info failed");
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
				...
				//LOG_INF("ov02b1b start read eeprom ---vivo_otp_read_when_power_on = %d\n", vivo_otp_read_when_power_on);
				//vivo_otp_read_when_power_on = ov02b1b_otp_read();
				//LOG_INF("ov02b1b read eeprom ---vivo_otp_read_when_power_on = %d,OV02B1B_OTP_ERROR_CODE=%d\n", vivo_otp_read_when_power_on, OV02B1b_OTP_ERROR_CODE);
				/*vivo lxd add end*/
				return ERROR_NONE;
			}
...
/*L19 code for 161200 by zhangmeng at 2021.11.24 start*/
...
/*L19 code for 161200 by zhangmeng at 2021.11.24 end*/
				  ...
				  UINT8 *feature_para, UINT32 *feature_para_len)
{
    ...
    *(feature_data + 1) = imgsensor_info.min_shutter;
	...
	case SENSOR_FEATURE_GET_CUSTOM_INFO:
	    printk("SENSOR_FEATURE_GET_CUSTOM_INFO information type:%lld  OV02B1B_OTP_ERROR_CODE:%d \n", *feature_data,OV02B1B_OTP_ERROR_CODE);
		switch (*feature_data) {
			...
			if (*feature_para_len >= sizeof(MUINT32)*13 + 2) {
			    *(MUINT32 *)(uintptr_t)(*(feature_data+1)) = OV02B1B_OTP_ERROR_CODE;//otp_state
				memcpy( feature_data+2, sn_inf_main_ov02b1b, sizeof(MUINT32)*13); 
				memcpy( feature_data+10, material_inf_main_ov02b1b, sizeof(MUINT32)*4); 
				#if 0
						for (i = 0 ; i<12 ; i++ ){
						printk("sn_inf_main_ov02b1b[%d]= 0x%x\n", i, sn_inf_main_ov02b1b[i]);
						}
				#endif
			}
				break;
