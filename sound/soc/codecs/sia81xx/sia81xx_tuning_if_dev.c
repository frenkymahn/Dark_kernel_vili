...
/* L19 code for HQ-179815 by zhangkaiming at 2022/02/25 start */
...
/* L19 code for HQ-179815 by zhangkaiming at 2022/02/25 end */
...
static unsigned long sia81xx_tuning_if_dev_type_open(uint32_t cal_id)
{
	...
	ret = record_cal_module(TUNING_FD_DUMMY, cal_id, SIXTH_SIA81XX_RX_MODULE);
	if (0 != ret) {
	...
	uint32_t mode_id, uint32_t param_id, uint32_t size, uint8_t *payload)
{
	...
	ret = write_payload_to_cal_module(module, OPT_GET_CAL_VAL, param_id, size, payload);
	if (0 != ret) {
...
//		OPT_SET_CAL_VAL, param_id, size, payload))) {
	ret = write_payload_to_cal_module(module, OPT_SET_CAL_VAL, param_id, size, payload);
	if (0 != ret) {
		pr_err("[  err][%s] %s: write_payload_to_cal_module ret = %d !! \r\n",
			LOG_FLAG, __func__, ret);
